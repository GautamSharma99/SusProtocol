...
export class OddsEngine {
   ...
   */
  private applyOddsDelta(market: MarketState, delta: Record<string, number>): void {
    let total = 0
  ...
  resolveMarket(market: MarketState, winningOutcome: string): boolean {
    if (market.isResolved) return false
    if (!market.outcomes.includes(winningOutcome)) return false
    ...
    const resolvedOdds: Record<string, number> = {}
    for (const outcome of market.outcomes) {
